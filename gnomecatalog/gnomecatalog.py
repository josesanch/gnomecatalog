# -*- coding: UTF-8 -*-
import sys, gtk, gobject, threading, pango
import config, utils

from widget.panel import Information_panel
...
class GnomecatalogApp(GladeConnect):
...
#		self.reload_categories()
	...
	def __create_widgets(self):
		...
		self.treeFiles = tree.Files(self.information_panel)
		self.treeDisks = tree.Disks(self.treeFiles, self.information_panel)
		self.hpaned1.pack1(ScrolledWindow(self.treeDisks))
		self.hpaned1.set_position(180)

		self.hpaned2 = gtk.HPaned()
		self.hpaned2.pack1(ScrolledWindow(self.treeFiles), True, True)

		# Creation of the information panel
		self.hpaned2.pack2(self.information_panel, False, True)
		self.hpaned2.show()
		self.hpaned1.pack2(self.hpaned2)
		self.menu = widget.menu.MainMenu(self)
		self.vbox_main.pack_start(self.menu.get_widget("/MenuBar"), False, False)
		self.vbox_main.reorder_child(self.menu.get_widget("/MenuBar"), 0)

#		self.toolbutton_add.set_menu(self.menu_toolbar_add);


#  		self.menu_information_panel.set_active(True)
	...
	def import_data(self):
		...
		disk = reader.Reader().read(path, self.progress_bar)
		self.hbox_import.hide()
		self.treeDisks.add(disk)
...
#			self.data.load_data()
	...
	def on_search_changed(self, entry, selected_last_selected = False):
		if entry.get_text() != "" and len(entry.get_text()) > 2:
...
#		self.data.search(entry.get_text(), self.get_selected_category())
		files = Files()
		self.treeFiles.set_data(files.search(entry.get_text()))
		return False
...
#		self.data.show_information()
	...
	def on_menu_abrir_activate(self, widget):
		database = dialogs.FileDialogs().open()

		ActiveRecord.database.open(database)
		self.config.save("database", database)
		disks = Disks()
		print disks.select()
#		self.treeDisks.set_data(disks.select())
...
#		self.data.close_database()
	...
	def on_menu_information_panel(self, widget):
